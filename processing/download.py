...
def download_data(data_name="www19", use_proxy=True):
    ...
    dataset_urls_dict = {
        ...
        "www2019": 'https://github.com/vonfeng/DPLink/archive/refs/heads/master.zip',
    ...
    if data_name=="tsmc2014" and not os.path.exists(os.path.join(TSMC2014_DATA_DIR, 'dataset_TSMC2014_NYC.txt')):
        ...
        r = requests.get(dataset_url)
        with open(os.path.join(DATA_PATH, "dataset_www2019.zip"), 'wb') as f:
            f.write(r.content)
        print('Download complete!')

        print('Extracting the zip folder...')
        os.system('unzip {} -d {}/'.format(os.path.join(DATA_PATH, "dataset_www2019.zip"), DATA_PATH))
        print('Extraction complete!')

        os.system('mv {}/DPLink-master/data {}/'.format(DATA_PATH, DATA_PATH))
        os.system('mv {} {}'.format(os.path.join(DATA_PATH, "data"), WWW2019_DATA_DIR))
        os.system('unzip {}/isp.zip -d {}/'.format(WWW2019_DATA_DIR, WWW2019_DATA_DIR))
        os.system('rm {}/isp.zip'.format(WWW2019_DATA_DIR))
        os.system('rm -r {}/DPLink-master/'.format(DATA_PATH))

        print('Removing the zip folder...')
        os.system('rm {}'.format(os.path.join(DATA_PATH, "dataset_www2019.zip")))
        print('Removal complete!')

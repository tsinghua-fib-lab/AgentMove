...
import json
import tqdm
from .config import MAX_TRAJ_LEN, SAVE_DIR

max_len = MAX_TRAJ_LEN  # max traj len; i.e., M
...
def haversine_torch(lonlat1, lonlat2):
    """
    Calculate the great circle distance between two vectors of points
    on the earth (specified in decimal degrees)
    """
...
def rs_mat2s(poi, l_max):
    ...
    for i, loc1 in tqdm.tqdm(enumerate(candidate_loc)):
...
def process_traj(dname):  # start from 1
    ...
    sorted_user_ids_trajs = sorted(zip(trajs, list(range(1, num_user+1))), key=lambda x: len(x[0]), reverse=True)
    _, sorted_user_ids = zip(*sorted_user_ids_trajs)
    ...
    data_pkl = os.path.join(SAVE_DIR,  dname + '_data.pkl')
    open(data_pkl, 'a')
    with open(data_pkl, 'wb') as pkl:
        joblib.dump(data, pkl)
    with open(os.path.join(SAVE_DIR, dname+"_sorted_user_ids.json"), "w") as fid:
        json.dump(sorted_user_ids, fid)
